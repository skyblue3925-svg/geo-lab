...
- **판정 키 커버리지**: 모든 지형이 공통 판정값(`relief`, `max_elevation`, `min_elevation`, `water_fraction`)을 받는다.
  지형 전용 키는 선상지 `fan_area_ratio`·`toe_area_ratio`, 리아스 해안 `flooded_valleys`·`sea_level` 등을 추가했다.
  공통값도 단계에 따라 변하지 않는 지형(곡류 `meander`, 버섯바위 `pedestal_rock`, 해식아치 `sea_arch` 등)은 전용 키가 더 필요하다.
- **해상도 의존 키**: 일부 생성기는 폭을 격자 칸 수로 그려서 `water_fraction` 같은 값이 해상도에 따라 달라진다.
  교사용 탭의 키 목록에 ⚠️ 로 표시되고, `tests/test_learning_specs.py` 가 번들 스펙의 판정을 해상도 40·60·100 에서 검사한다.
