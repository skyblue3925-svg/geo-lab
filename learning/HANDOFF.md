...
5. **모션그래픽형 표현(프로토타입).** `prototypes/motion/meander_motion.py` 가 자유곡류 지형 위에 물 입자, 단계별 이름표, 단면도, 카메라 이동을 얹어 MP4 로 내보낸다.
   직접 제작하는 모션그래픽 대신 생성기 데이터로 자동 생성하는 방식이다. 다음은 이 과정 레이어를 갤러리·학습 페이지에 넣고 '영상 내보내기' 버튼을 다는 것.
   실행에는 `pip install imageio-ffmpeg` 가 필요하다.
6. **LLM 호출을 앱 안에 넣을지 결정한다.** 지금은 API 키가 필요 없도록 프롬프트 복사 방식이다.
7. **학습 기록을 저장할지 결정한다.** 지금은 세션에만 남는다. 필요하면 방문자 카운터처럼 Supabase 에 붙일 수 있다.
